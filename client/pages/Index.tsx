...
import RoomRow, { CATEGORY_RATES, RoomCategory, RoomRowData, computeRow } from "@/components/invoice/RoomRow";
...
export default function Index() {
  ...
  const [rows, setRows] = useState<RoomRowData[]>([
    computeRow({ id: uid(), category: "" as any, rate: 0, rooms: defaultRooms, nights: defaultNights, subtotal: 0, sgst: 0, cgst: 0, total: 0 })
  ]);

  useEffect(() => {
    // If defaults change, apply to rows that are zero
    setRows((rs) => rs.map((r) => computeRow({ ...r, rooms: r.rooms || defaultRooms, nights: r.nights || defaultNights })));
  ...
  const updateRow = (id: string, updated: RoomRowData) => setRows((rs) => rs.map((r) => (r.id === id ? computeRow(updated) : r)));
  ...
  const input = "w-full rounded-md border bg-white px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/40";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
      ...
      <main className="container py-6 space-y-8">
        ...
        <section className={card + " p-5"}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input className={input} value={hotel.name} onChange={(e) => setHotel({ ...hotel, name: e.target.value })} />
            </div>
            <div>
              <label className={label}>GSTIN</label>
              <input className={input} value={hotel.gstin} onChange={(e) => setHotel({ ...hotel, gstin: e.target.value })} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={label}>Phone</label>
                <input className={input} value={hotel.phone} onChange={(e) => setHotel({ ...hotel, phone: e.target.value })} />
              </div>
              <div>
                <label className={label}>Email</label>
                <input className={input} value={hotel.email} onChange={(e) => setHotel({ ...hotel, email: e.target.value })} />
              </div>
            </div>
            <div className="md:col-span-3">
              <label className={label}>Address</label>
              <textarea className={input} rows={2} value={hotel.address} onChange={(e) => setHotel({ ...hotel, address: e.target.value })} />
            </div>
          </div>
        </section>
        ...
        <section className={card + " p-5"}>
          <h2 className="text-sm font-semibold tracking-wide mb-4">Guest & Invoice Details</h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input className={input} placeholder="e.g. RES12345" value={folioNo} onChange={(e) => setFolioNo(e.target.value)} />
            </div>
            <div>
            ...
            <div>
              ...
              <input className={input} placeholder="Company / Person" value={billTo} onChange={(e) => setBillTo(e.target.value)} />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" min={0} className={input} value={defaultNights} onChange={(e) => setDefaultNights(Number(e.target.value || 0))} />
            </div>
            <div>
              <label className={label}>No. of Rooms</label>
              <input type="number" min={0} className={input} value={defaultRooms} onChange={(e) => setDefaultRooms(Number(e.target.value || 0))} />
            </div>
            <div>
        ...
        <section className={card + " p-5 space-y-4"}>
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold tracking-wide">Room Details</h2>
            <Button onClick={addRow} type="button" className="bg-primary hover:bg-primary/90 text-primary-foreground">
              <Plus className="w-4 h-4" /> Add Room
            </Button>
          </div>
          <div className="space-y-3">
            {rows.map((r) => (
              <RoomRow key={r.id} data={r} onChange={(u) => updateRow(r.id, u)} onRemove={() => removeRow(r.id)} />
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
            <div className="md:col-span-2 text-sm text-muted-foreground">
              Category default rates: {Object.entries(CATEGORY_RATES).map(([k, v]) => `${k} = ₹${v}/night`).join(" • ")}
            </div>
            <div className="md:col-span-1">
              <div className="border rounded-md overflow-hidden">
                ...
                <div className="flex items-center justify-between p-2 border-b">
                  ...
                  <span className="font-medium">{formatINR(totals.sgst)}</span>
                </div>
                <div className="flex items-center justify-between p-2 border-b">
                  <span className="text-sm">CGST (2.5%)</span>
                  <span className="font-medium">{formatINR(totals.cgst)}</span>
                </div>
                <div className="flex items-center justify-between p-2 bg-primary/5">
                  <span className="text-sm font-semibold">Grand Total</span>
                  <span className="text-base font-bold text-primary">{formatINR(totals.grand)}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <footer className="py-8 text-center text-xs text-muted-foreground">
        Built with love for hospitality • Accent color #f77f00 • Responsive & printable
      </footer>
    </div>
  );
}
